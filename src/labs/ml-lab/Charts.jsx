...
export function DataPlot({ train, validation, model, reference }) {
  ...
  return <svg viewBox="0 0 560 325" role="img" aria-label="Training circles, validation diamonds, current prediction line and dashed least-squares reference">
    ...
    {[0,1,2,3,4].map(i => { const y=y0+(y1-y0)*i/4, x=x0+(x1-x0)*i/4; return <g key={i}>
      <line x1="54" y1={Y(y)} x2="534" y2={Y(y)} stroke="#263747" />
    ...
    <g clipPath="url(#ml-plot-clip)">
      {train.slice(0,100).map((p,i) => <line key={`e${i}`} x1={X(p.x)} x2={X(p.x)} y1={Y(p.y)} y2={Y(predict(p.x,model.w,model.b))} stroke="#51c5da" opacity="0.2" />)}
      <path d={line(reference.w,reference.b)} fill="none" stroke="#b4b6ca" strokeDasharray="6 5" strokeWidth="2" />
      <path d={line(model.w,model.b)} fill="none" stroke="#5eead4" strokeWidth="3" />
      {train.map((p,i) => <circle key={i} cx={X(p.x)} cy={Y(p.y)} r="3.5" fill="#57b9f7"><title>{`Training: x=${p.x}, y=${p.y}`}</title></circle>)}
      {validation.map((p,i) => <path key={i} d={`M ${X(p.x)} ${Y(p.y)-4} l 4 4 l -4 4 l -4 -4 Z`} fill="#ffb86b"><title>{`Validation: x=${p.x}, y=${p.y}`}</title></path>)}
    </g>
...
export function LossPlot({ history }) {
  ...
  return <svg viewBox="0 0 560 190" role="img" aria-label="Training and validation mean squared error by iteration, logarithmic one-plus-loss scale">
    {[0,0.5,1].map(f => <g key={f}><line x1="54" x2="534" y1={150-f*120} y2={150-f*120} stroke="#263747" /><text x="47" y={154-f*120} textAnchor="end">{(10**(f*max)-1).toPrecision(2)}</text></g>)}
    {['train','validation'].map((key,i) => <polyline key={key} points={history.map(h => `${X(h.step)},${Y(h[key])}`).join(' ')} fill="none" stroke={i ? '#ffb86b' : '#57b9f7'} strokeWidth="2.5" strokeDasharray={i ? '6 3' : undefined} />)}
    <circle cx={X(history.at(-1).step)} cy={Y(history.at(-1).train)} r="3" fill="#57b9f7" />
