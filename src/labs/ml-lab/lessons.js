...
export const lessons = [
  {
    id: 'arrays',
    sections: ["One operation, two meanings","Read the shape","What is a weight?","Pair each input with its weight","Build a weighted sum","Connect it to the dot product","From one row to many","Avoid a silent shape mistake","Put it into practice","Before you start: how the Python cells work"], title: '00a · From Python lists to vectors', skill: 'Read array shapes and connect a weighted sum to a prediction.',
    ...
    paragraphs: [
      ...
      'The grey boxes with code are **Python cells** that run here in your browser. Run one with its button, or press **Shift + Enter** while editing it. The first run downloads Python (about 10 MB, then cached), so it takes a few seconds. Cells in one lesson form one **notebook**: a variable made in one cell exists in the others, but only once the cell that makes it has run — and it stays in Python’s memory, even after you change the code that made it, until you **restart** Python for that notebook. What is kept: your code edits are saved in this browser; outputs last for this visit; Python’s variables last until a restart, **Stop**, leaving the ML Lab, or 10 minutes without a run. When a cell fails, read the **last line** of the error first: it names the kind of problem and the name or line involved.',
    ],
  ...
  },
  {
    ...
    derivation: {
      title: 'Derive ∂(e²)/∂w and ∂(e²)/∂b for one observation',
      intro: 'Work through the chain rule one link at a time. Write every answer in terms of w, x, b and y.',
      steps: [
        { prompt: 'Write the prediction error e = ŷ − y.', answer: 'w*x + b - y', show: '`e = wx + b − y`', vars: { w: [-2, 2], x: [-3, 3], b: [-2, 2], y: [-3, 3] } },
        { prompt: 'The derivative of e² with respect to e is 2e. Write it in terms of w, x, b and y.', answer: '2*(w*x + b - y)', show: '`2(wx + b − y)`', vars: { w: [-2, 2], x: [-3, 3], b: [-2, 2], y: [-3, 3] } },
        { prompt: 'How fast does e change when w changes (∂e/∂w)?', answer: 'x', show: '`x`', why: 'w multiplies x, and b and y do not depend on w.', vars: { w: [-2, 2], x: [-3, 3], b: [-2, 2], y: [-3, 3] }, hint: 'Treat x, b and y as constants and differentiate wx + b − y with respect to w.' },
        { prompt: 'Multiply the two rates (chain rule): ∂(e²)/∂w = ?', answer: '2*(w*x + b - y)*x', show: '`2(wx + b − y)·x`', vars: { w: [-2, 2], x: [-3, 3], b: [-2, 2], y: [-3, 3] } },
        { prompt: 'Now the bias: ∂(e²)/∂b = ?', answer: '2*(w*x + b - y)', show: '`2(wx + b − y)`', why: '∂e/∂b = 1, so only the first factor remains.', vars: { w: [-2, 2], x: [-3, 3], b: [-2, 2], y: [-3, 3] }, hint: 'Same chain as before, but ∂e/∂b replaces ∂e/∂w.' },
        { prompt: 'Evaluate ∂(e²)/∂w at x = 2, y = 5, w = 1, b = 0.', number: -12, show: '`−12`', why: 'e = −3, so 2 × (−3) × 2 = −12.' },
      ],
