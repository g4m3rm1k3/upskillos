...
- `notebook/`: the lesson notebooks. `LessonNotebook.jsx` (cells, Run cell / Run all / Stop / Restart, per-cell run numbers, “edited since it ran”, tracebacks with plain-language hints, figures, `.ipynb` download and Copy to Notebook Lab — both export the learner’s edited cells); `runtime.js` (one shared worker, runs executed in order, Stop terminates the worker and bumps a generation so each notebook knows its variables are gone); `notebook.worker.js` (Pyodide 0.26.4, one globals dict per notebook — cells of a notebook share state, notebooks are isolated — imports loaded on demand, Agg matplotlib captured as PNG, tracebacks trimmed to the learner’s cell); `drafts.js` (edits saved per lesson under `upskillos.ml-lab.notebooks.v1` with a fingerprint of the original cells, so a course update is reported and the learner chooses; outputs kept for the page session). Stop discards every notebook’s variables but never code.
