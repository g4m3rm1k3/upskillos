...
while True:
  ...
  },
  {
    ...
    blocks: [
      { p: 0 },
      { figure: 'OptimalityBackup', caption: 'Pick a cell: each action’s one-step lookahead under $V^*$, and the largest of them.' },
      { p: 1 },
      { figure: 'ValueIterationSweeps', caption: 'Value iteration from $V = 0$, one sweep at a time, with the greedy arrows for the current values.' },
      { cell: 1 },
      { cell: 2 },
      { p: 2 },
      { figure: 'ContractionPlot', caption: 'The largest change in each sweep (log scale) against the guarantee $\\gamma^k$ times the first change.' },
      { predict: { prompt: 'With $\\gamma = 0.9$ and a starting error of 10, after how many sweeps does the bound $10 \\times 0.9^k$ guarantee an error below 1? (The smallest whole number.)', answer: 22, explain: '$0.9^k < 0.1$ needs $k > \\ln 10 / \\ln(1/0.9) \\approx 21.85$, so 22 sweeps. Check: $10 \\times 0.9^{21} \\approx 1.09$, $10 \\times 0.9^{22} \\approx 0.98$.', misconceptions: [{ answer: 21, feedback: '$10 \\times 0.9^{21} \\approx 1.09$ is still above 1. One more sweep.' }] } },
      { p: 3 },
      { figure: 'RouteBySlip', caption: 'The optimal values, arrows and route as slip rises. Outlined: the path when no slip happens.' },
      { p: 4 },
      { p: 5 },
      { figure: 'BaselineTable', caption: 'Random moves, the hand-written route and the optimal policy, each run for 400 episodes.' },
      { math: true },
      { derivation: true },
    ],
...
for sweep in range(max_sweeps):
  ...
  },
  {
    ...
    blocks: [
      { p: 0 },
      { p: 1 },
      { figure: 'TDStep', caption: 'One update: the estimate moves a fraction α of the way to the target.' },
      { predict: { prompt: '$Q(s, a) = 1$, $R = -0.1$, $\\gamma = 0.9$, the next state’s best $Q$ is 3, and $\\alpha = 0.25$. What is the updated $Q(s, a)$?', answer: 1.4, tolerance: 0.001, explain: 'Target $= -0.1 + 0.9 \\times 3 = 2.6$; $\\delta = 2.6 - 1 = 1.6$; new $Q = 1 + 0.25 \\times 1.6 = 1.4$.', misconceptions: [{ answer: 2.6, feedback: 'That is the target. With $\\alpha = 0.25$ the estimate moves only a quarter of the way there.' }, { answer: 1.6, feedback: 'That is the TD error $\\delta$. Multiply it by $\\alpha$ and add it to the old estimate.' }] } },
      { figure: 'RunningAverage', caption: 'Noisy targets averaged two ways: $\\alpha = 1/n$ and a constant $\\alpha$.' },
      { cell: 1 },
      { p: 2 },
      { figure: 'QversusSarsa', caption: 'The greedy route each method learns while exploring with ε. Rows are numbered from the top; the ditch is row 4.' },
      { p: 3 },
      { figure: 'MaxBias', caption: 'Maximization bias with actions that are all truly worth 0.' },
      { cell: 2 },
      { p: 4 },
      { figure: 'TrainingLength', caption: 'How often the greedy policy reaches the star after training for n episodes, for five seeds.' },
      { p: 5 },
      { math: true },
      { derivation: true },
    ],
...
S_t = S_next`,
  ...
  },
  {
    ...
    blocks: [
      { p: 0 },
      { p: 1 },
      { figure: 'LoopPath', caption: 'The route value iteration chooses, with and without the re-collectable bonus.' },
      { predict: { prompt: 'Circling earns +2 every 2 moves and costs 0.1 per move. Ignoring discounting, what does circling earn over **20** moves?', answer: 18, explain: '10 checkpoint entries × 2 − 20 × 0.1 = 20 − 2 = 18: already more than the +10 star minus 0.8 for the 8-move route.' } },
      { p: 2 },
      { figure: 'HackingCurves', caption: 'Q-learning’s training reward and the task reward, episode by episode.' },
      { p: 3 },
      { figure: 'Telescoping', caption: 'Every shaping term along a path. The sum depends only on where the path starts and ends.' },
      { predict: { prompt: 'With $\\gamma = 1$, a path starts at a cell with $\\Phi(S_0) = -7$ and ends at the star, where $\\Phi = 0$. What is the total shaping bonus along the path?', answer: 7, explain: '$\\gamma^T\\Phi(S_T) - \\Phi(S_0) = 0 - (-7) = 7$, whatever route the path took.', misconceptions: [{ answer: -7, feedback: 'Watch the sign: the total is $\\Phi(S_T) - \\Phi(S_0) = 0 - (-7)$.' }] } },
      { cell: 1 },
      { p: 4 },
      { figure: 'ShapingKeepsPolicy', caption: 'Solve the task with an added reward and count the cells where the chosen action is no longer optimal for the task.' },
      { p: 5 },
      { math: true },
      { derivation: true },
    ],
    formulaTex: '$$R_{\\text{train}} \\uparrow,\\ R_{\\text{task}} \\downarrow \\;\\Rightarrow\\; \\text{misspecified}$$ $$r\' = r + \\gamma\\Phi(s\') - \\Phi(s)$$ $$\\sum_{k=0}^{T-1}\\gamma^k F_k = \\gamma^T\\Phi(S_T) - \\Phi(S_0)$$',
