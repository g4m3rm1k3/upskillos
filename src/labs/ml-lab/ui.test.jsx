...
describe('learning workspace interactions',()=>{
  it('does not count a wrong answer, and persists a correct numeric checkpoint',()=>{
    ...
    expect(screen.getAllByRole('status')[0].textContent).toContain('Not quite')
  ...
  it('explains a wrong decision and records a right one',()=>{
    ...
    expect(screen.queryByText(/Counting wins throws away the sizes/)).toBeNull()   // nothing is revealed before submitting
    fireEvent.click(screen.getByText('Check answer',{selector:'button'}))
    expect(screen.getByText(/Counting wins throws away the sizes/)).toBeTruthy()
    const stored=()=>JSON.parse(localStorage.getItem('upskillos.ml-lab.v1')).progress['l33-evidence']
    expect(stored().passed).toBeFalsy()
    fireEvent.click(options[2])
    expect(stored().passed).toBeFalsy()                                            // selecting alone records nothing
    fireEvent.click(screen.getByText('Check answer',{selector:'button'}))
    expect(stored()).toMatchObject({passed:true,help:'feedback',attempts:2})
    expect(screen.getByText(/Answered after feedback/)).toBeTruthy()
  })
  it('records an unaided answer, accepts a fraction, and marks answers given after the explanation',()=>{
    render(<MLLab />)
    fireEvent.change(screen.getByLabelText('Choose lab'),{target:{value:'3'}})
    fireEvent.click(screen.getByRole('button',{name:'03.7 · Units, step sizes and standardization'}))
    fireEvent.change(screen.getByLabelText('Checkpoint answer'),{target:{value:'1/4'}})
    fireEvent.click(screen.getByText('Check answer',{selector:'button'}))
    const stored=id=>JSON.parse(localStorage.getItem('upskillos.ml-lab.v1')).progress[id]
    expect(stored('l03-scaling')).toMatchObject({passed:true,help:'none',attempts:1})
    expect(screen.getByText(/Answered without help/)).toBeTruthy()
    fireEvent.click(screen.getByText('Next lesson →',{selector:'button'}))
    const details=screen.getByText('Worked explanation').closest('details')
    details.open=true; fireEvent(details,new Event('toggle'))
    fireEvent.change(screen.getByLabelText('Checkpoint answer'),{target:{value:'400'}})
    fireEvent.click(screen.getByText('Check answer',{selector:'button'}))
    expect(stored('l03-conditioning')).toMatchObject({passed:true,help:'explanation'})
  })
  it('does not count unreadable input as an attempt',()=>{
    render(<MLLab />)
    fireEvent.change(screen.getByLabelText('Checkpoint answer'),{target:{value:'23%'}})
    fireEvent.click(screen.getByText('Check answer',{selector:'button'}))
    expect(screen.getByText(/plain number, not a percentage/)).toBeTruthy()
    expect(JSON.parse(localStorage.getItem('upskillos.ml-lab.v1')).progress.arrays).toBeUndefined()
  })
  it('names a likely slip for a wrong numeric answer',()=>{
    render(<MLLab />)
    fireEvent.change(screen.getByLabelText('Checkpoint answer'),{target:{value:'-23'}})
    fireEvent.click(screen.getByText('Check answer',{selector:'button'}))
    expect(screen.getByText(/opposite sign/)).toBeTruthy()
  })
  it('routes a prerequisite gap to the lesson that teaches it and back',()=>{
    ...
    expect(screen.getByText('Check my answers',{selector:'button'}).disabled).toBe(true)          // all three first
    fireEvent.click(screen.getByRole('group',{name:'Prerequisite check 2'}).querySelectorAll('input')[1])
    fireEvent.click(screen.getByRole('group',{name:'Prerequisite check 3'}).querySelectorAll('input')[2])
    expect(screen.queryByText(/Lab 03 · 03.2 · Matrix multiplication/,{selector:'button'})).toBeNull()
    fireEvent.click(screen.getByText('Check my answers',{selector:'button'}))
    expect(screen.getByText(/Revisit these lessons/)).toBeTruthy()
    fireEvent.click(screen.getAllByText(/Lab 03 · 03.2 · Matrix multiplication/,{selector:'button'})[0])
