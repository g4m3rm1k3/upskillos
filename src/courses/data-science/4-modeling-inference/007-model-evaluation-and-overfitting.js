  ...
  intuition:{
    ...
    visualizations:[{id:'PythonNotebook',title:'Model Evaluation',props:{initialCells:[
      ...
      {id:4,cellTitle:'Stage 4 — Bias-Variance Tradeoff',
       prose:'Underfitting = high bias. Overfitting = high variance. The best model balances both. With only 20 training points, training error keeps falling as the degree grows, while validation error falls and then rises sharply once the curve starts chasing noise. (StandardScaler rescales x before the powers are taken; without it, x¹¹ for x near 5 is enormous and the fit becomes numerically unreliable.)',
       instructions:'Run. Look for the rough U-shape in val_err. With noisy data the curve is bumpy and neighbouring degrees can be nearly tied, so the "best degree" is an estimate from this one split, not a guaranteed right answer. Change random_state in the split and see whether it moves.',
       code:'import numpy as np\nfrom sklearn.model_selection import train_test_split\nfrom sklearn.linear_model import LinearRegression\nfrom sklearn.preprocessing import PolynomialFeatures, StandardScaler\nfrom sklearn.pipeline import make_pipeline\nnp.random.seed(42)\nX=np.random.uniform(0,5,40).reshape(-1,1)\ny=np.sin(X.ravel())+np.random.normal(0,0.3,40)\nX_tr,X_val,y_tr,y_val=train_test_split(X,y,test_size=0.5,random_state=0)\ndegrees=list(range(1,13))\ntrain_err=[]\nval_err=[]\nfor d in degrees:\n    m=make_pipeline(StandardScaler(),PolynomialFeatures(d),LinearRegression())\n    m.fit(X_tr,y_tr)\n    train_err.append(1-m.score(X_tr,y_tr))\n    val_err.append(1-m.score(X_val,y_val))\nbest=degrees[val_err.index(min(val_err))]\nprint(f"Best degree: {best}")\nfor d,tr,va in zip(degrees,train_err,val_err):\n    print(f"  degree {d:>2}: train_err={tr:.3f}, val_err={va:.3f}")',output:'',status:'idle'},
